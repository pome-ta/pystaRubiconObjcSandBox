...
from pyrubicon.objc.api import ObjCInstance, ObjCClass, ObjCProtocol, objc_property, objc_method,objc_classmethod
...
def present_ViewController(viewController_instance):
  ...
  nv = WrapNavigationController.initWithRootViewControllerOnMianThread_(vc)
...
class WrapNavigationController(UINavigationController,
  ...
  def _init(self, vc:UIViewController):
    nv = self.alloc().initWithRootViewController_(vc)
    nv.delegate = self
    return nv
    

  @classmethod
  def initWithRootViewControllerOnMianThread_(cls, vc: UIViewController):
    _cls = cls.init(vc)
    return _cls
