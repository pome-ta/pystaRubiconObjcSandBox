...
from pyrubicon.objc.runtime import objc_id, send_super, objc_block, load_library, libobjc, Class, Foundation, SEL
...
def present_viewController(viewController: ObjCInstance,
  ...
  if (sheet := presentViewController.sheetPresentationController()):
    sheet.setDetents_([
      #mediumDetent,
      largeDetent,
    ])
  ...
  @Block
  def completion() -> None:
    print('present_completion: block')

  rootViewController.presentViewController_animated_completion_(
    presentViewController, True, completion)
  #rootViewController.presentViewController_animated_completion_(sheet, True, completion)
