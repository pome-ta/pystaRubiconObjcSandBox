...
# [PEP 3156 – Asynchronous IO Support Rebooted: the “asyncio” Module | peps.python.org](https://peps.python.org/pep-3156/)
...
CFSocketCallback = CFUNCTYPE(None, CFSocketRef, CFSocketCallbackType,
                             CFDataRef, c_void_p, c_void_p)
...
libcf.CFRunLoopAddSource.argtypes = [
  CFRunLoopRef, CFRunLoopSourceRef, CFRunLoopMode
]

libcf.CFRunLoopAddTimer.restype = None
libcf.CFRunLoopAddTimer.argtypes = [
  CFRunLoopRef, CFRunLoopTimerRef, CFRunLoopMode
]
...
libcf.CFRunLoopRemoveSource.argtypes = [
  CFRunLoopRef, CFRunLoopSourceRef, CFRunLoopMode
]

libcf.CFRunLoopRemoveTimer.restype = None
libcf.CFRunLoopRemoveTimer.argtypes = [
  CFRunLoopRef, CFRunLoopTimerRef, CFRunLoopMode
]
...
libcf.CFSocketCreateRunLoopSource.argtypes = [
  CFAllocatorRef, CFSocketRef, CFIndex
]
...
class CFTimerHandle(events.TimerHandle):
  ...
  def __init__(self, *, loop, timeout, callback, args):
    ...
    libcf.CFRunLoopAddTimer(self._loop._cfrunloop, self._timer,
                            kCFRunLoopCommonModes)

  def cancel(self):
    """Cancel the Timer handle."""
    super().cancel()
    libcf.CFRunLoopRemoveTimer(self._loop._cfrunloop, self._timer,
                               kCFRunLoopCommonModes)
    self._loop._timers.discard(self)
...
class CFSocketHandle(events.Handle):
  ...
  def _cf_socket_callback(self, cfSocket, callbackType, ignoredAddress,
                          ignoredData, context):
    if self._fd not in self._loop._sockets:
      ...
      libcf.CFRunLoopRemoveSource(self._loop._cfrunloop, self._src,
                                  kCFRunLoopCommonModes)
      self._src = None
      return
  ...
  def __init__(self, *, loop, fd):
    """Register a file descriptor with the CFRunLoop, or modify its state so that it's listening for both notifications (read and write) rather than just one; used to implement add_reader and add_writer."""
    ...
    self._src = libcf.CFSocketCreateRunLoopSource(kCFAllocatorDefault,
                                                  self._cf_socket, 0)
    libcf.CFRunLoopAddSource(self._loop._cfrunloop, self._src,
                             kCFRunLoopCommonModes)
...
class CFEventLoop(unix_events.SelectorEventLoop):
  ...
  def run(self):
    ...
    if (not recursive and hasattr(events, "_get_running_loop")
  ...
  def run_forever(self, lifecycle=None):
    ...
    if self.is_running():
      raise RuntimeError("Recursively calling run_forever is forbidden. "
