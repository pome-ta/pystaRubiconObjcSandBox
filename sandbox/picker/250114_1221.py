...
#[Pythonista/_2017/picker-wheel-for-lists.py at master · tdamdouni/Pythonista · GitHub](https://github.com/tdamdouni/Pythonista/blob/master/_2017/picker-wheel-for-lists.py)
...
_data = [[str(x) for x in range(2000, 2040)], ['xxxx', 'yyyy', 'zzzz']]
...
_default_attributes = {
  _str_symbol('NSFontAttributeName'):
  UIFont.fontWithName_size_(ns('DBLCDTempBlack'), 128),
  'NSColor':
  UIColor.redColor(),
  _str_symbol('NSBackgroundColorAttributeName'):
...
#print(type(_str_symbol('NSForegroundColorAttributeName')))
print(_str_symbol('NSBackgroundColorAttributeName'))
#NSColor
#print(type(ns(_default_attributes)))
#print(ns(_default_attributes))
...
def pickerView_attributedTitleForRow_forComponent_(self, cmd, picker_view, row,
  ...
  titile = NSAttributedString.alloc().initWithString_attributes_(ns(_data[tag - 1][row]), ns(_default_attributes))
  #print(titile)
  return titile.ptr
  #return NSAttributedString.alloc().initWithString_attributes_('hoge', ns(_default_attributes)).ptr
...
def pickerView_didSelectRow_inComponent_(self, cmd, picker_view, row,
                                         component):
  tag = ObjCInstance(picker_view).tag()
...
class MyView(ui.View):
  ...
  def __init__(self, **kwargs):
    ...
    self.delegate_and_datasource = UIPickerViewDataSourceAndDelegate.alloc(
    ).init().autorelease()
